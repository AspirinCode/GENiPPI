...
import sys
sys.path.append(r'/home/jmwang/WorkSpace/GENiPPI/gPPMol')
import os
from rdkit.Chem.rdmolfiles import MolFromPDBFile
from ppi_processing.Extract_Interface import Extract_Interface
from ppi_processing.Feature_Processing import get_atom_feature
import numpy as np
...
def Prepare_Input(structure_path):
    ...
    print(receptor_path, ligand_path)
    ...
    #import pandas as pd    
    #df = pd.DataFrame(adj1)
    #print(df)
    #element = [atom.GetSymbol() for atom in receptor_mol.GetAtoms()]
    #df_e = pd.DataFrame(adj1, index=element,  columns=element)
    #print(df_e)
    d3 =  np.vstack([d1,d2])   

    max_ = np.max(d3,axis=0)    #[85.624     39.388       69.581]
    min_ = np.min(d3,axis=0)    #[38.59      3.121        24.963]

    center = (max_-min_)/2      #[23.517     18.1335      22.309 ]
    length = max_ - min_        #[47.034     36.267       44.618]
